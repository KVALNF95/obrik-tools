...
Что делает:
  ...
  3. Записывает Beacon Delay = Infinite во все ESC (требуется АКБ)
  4. Загружает параметры в полётник (через QGC или MAVLink param_set)
...
import os, sys, time, re, glob, subprocess, argparse, json, textwrap
from pathlib import Path

# ── конфиг по умолчанию ──────────────────────────────────────────────
DEFAULT_CONFIG = {
    "bootloader":   f"{os.path.expanduser('~')}/Documents/Applications/px4/matek_h743-slim_bootloader.bin",
    "firmware":     f"{os.path.expanduser('~')}/Documents/Applications/px4/PX4-Autopilot/build/matek_h743-slim_default/matek_h743-slim_default.px4",
    "params_file":  f"{os.path.expanduser('~')}/Downloads/stable_o2.params",
    "px4_tools":    f"{os.path.expanduser('~')}/Documents/Applications/px4/PX4-Autopilot/Tools",
...
def serial_by_id():
    """Путь к полётнику, не зависящий от номера ACM."""
    ids = sorted(glob.glob("/dev/serial/by-id/usb-Matek*"))
...
def run(cmd, check=True, **kw):
    """Запустить команду с индикацией."""
    print(f"  $ {cmd}")
...
def step_beacon_delay(cfg):
    """Шаг 3: записать Beacon Delay = Infinite во все ESC."""
    beacon_val = cfg.get("beacon_value", "5")
    num = int(cfg.get("num_motors", "4"))

    print("\n" + "=" * 60)
    print("ШАГ 3 — отключение писка регуляторов (Beacon Delay)")
    print("=" * 60)
...
def step_load_params(cfg):
    """Шаг 4: загрузить параметры в полётник через MAVLink (param_set)."""
    ...
    print("\n" + "=" * 60)
    print("ШАГ 4 — загрузка параметров (MAVLink param_set)")
    print("=" * 60)
...
def main():
    ...
    parser.add_argument("--steps", "-s", default="1,2,3,4",
                        help="какие шаги выполнить: 1=загрузчик, 2=прошивка, 3=beacon, 4=параметры, либо 'beacon', 'fw', 'all'")
    ...
    for step_num in sorted(do):
        if step_num == 1:
            ...
            results[3] = step_beacon_delay(cfg)
        elif step_num == 4:
            results[4] = step_load_params(cfg)
        else:
    ...
    print("\n" + "=" * 60)
    print("ИТОГ")
    names = {1: "загрузчик", 2: "прошивка", 3: "Beacon Delay", 4: "параметры"}
    all_ok = True
    for step_num in sorted(do):
